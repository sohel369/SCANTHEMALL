...
function initAds() {
    // Verified reliable high-end Unsplash images
    const adInventory = {
        leaderboard: [
    ...
    placeholders.forEach((el) => {
        // Detect shape from container dimensions
        ...
        const rotate = () => {
            ...
            // 1. Create a dummy image to preload
            const img = new Image();
            img.src = nextImgUrl;
            
            // 2. While loading, show a premium skeleton/shimmer
            el.innerHTML = `
        ...
        rotate();
        setInterval(rotate, 15000); // 15 sec rotation
    });
}
