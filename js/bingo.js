...
function renderBingoGrid() {
    ...
    container.innerHTML = roundIndicator + grid.map((val, i) => {
        ...
        <div class="aspect-square rounded-lg flex items-center justify-center text-[10px] font-black relative group
            ${val ? 'bg-gradient-to-br from-[#FF3D00] to-[#E63700] text-white shadow-[0_4px_12px_rgba(255,61,0,0.4)]' : 'bg-zinc-900/50 text-zinc-600 border border-white/10'} 
            ${isHighlighted ? 'ring-2 ring-yellow-400 ring-offset-2 ring-offset-zinc-950 z-10 animate-pulse' : ''}
            transition-all duration-300 hover:scale-110 hover:border-[#FF3D00]/30">
            ${val ? `<i data-lucide="${icon}" class="w-5 h-5 opacity-30 absolute"></i><span class="relative z-10">★</span>` : `<i data-lucide="${icon}" class="w-4 h-4 opacity-20"></i>`}
            ${isHighlighted ? '<div class="absolute inset-0 bg-yellow-400/10 rounded-lg"></div>' : ''}
        </div>
    `}).join('');
